...
// La classe Saisie hérite des comportements du composant JDialog
public class Saisie extends javax.swing.JDialog {
   ...
   private String numDomicile="inconnu", numPortable="inconnu", courriel="inconnu";
   private String rue="inconnu", ville="inconnu";


  // Constructeur d'un composant de type JDialog, si modal = true, la fenêtre s'ouvre
  // et l'appllicaton reste bloquée tant que la fenêtre reste ouverte
  public Saisie(java.awt.Frame parent, boolean modal) {
        // Appeler le constructeur de la classe parente (de type JDialog)
        super(parent, modal);
        // Afficher les composants créés en mode Design
        initComponents();
        // Interdire la possibilité d'agrandir ou de réduire la fenêtre
        setResizable(false);
    }


    // Définition les méthodes en set
    ...
    // afin de récupérer les données mises à jour par la boîte de dialogue
    // par la classe Adresses_view
    ...
    private void initComponents() {
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(CarnetModele.Adresses_App.class).getContext().getResourceMap(Saisie.class);
        ...
        photoLbl.setIcon(resourceMap.getIcon("photoLbl.icon")); // NOI18N
        photoLbl.setText(resourceMap.getString("photoLbl.text")); // NOI18N
    ...
    // Les données saisies dans les champs de texte sont stockées
    // dans les propriétés de la classe, grâce aux méthodes en set
    ...
    // Lorsque l'utilisateur clique sur le bouton validerBtn
    private void validerBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_validerBtnActionPerformed
        ...
        setDonnees();
        // Une chaine de caractères infos est fabriquée par concaténation des propriétés (ordonnées)
        // et de point-virgue
        String infos = prenom + ";" + nom +";"+url+ ";" + numDomicile + ";" + numPortable + ";" + courriel + ";" + rue + ";" + ville;
        // la chaine infos est enregistrée dans le fichier de données
        ecrireLeContact(infos);
        // La fenêtre est effacée
        dispose();
}//GEN-LAST:event_validerBtnActionPerformed
   // Enregistrer le contact passé en paramètre dans le fichier de données
   private void  ecrireLeContact(String tmp) {
       // Lire le fichier listeContact.txt et mémoriser le contenu dans une liste
      lireLesContacts();
   ...
   private void ecrireLesContacts() {
    ...
    // Supprimer le contact dont l'identifiant est passé en paramètre
    public void  supprimer(int tmp) {
      // Lire le fichier listeContact.txt et mémoriser le contenu dans la liste listeContacts
      lireLesContacts();
      // Supprimer contact positionné à l'indice tmp
    ...
    private void lireLesContacts(){
        ...
        // tant que ...
        do {
            // Lire la chaine de caractère courante
           chaine = f.lire();
	   if (chaine != null){
               // Ajouter la chaine à liste des contacts de type ArrayList
