...
// Model for Etudiant entity
    models.Etudiant = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/ServiceWebRESTInscription/api/etudiants/",
        idAttribute: 'id',
        defaults: {
            idFiliere: "",
            cne: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/ServiceWebRESTInscription/api/etudiants/';
            }
    ...
    // Collection class for Etudiant entities
    models.EtudiantCollection = Backbone.Collection.extend({
        model: models.Etudiant,
        url: "http://localhost:8080/ServiceWebRESTInscription/api/etudiants/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.EtudiantCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-etudiant-list-itemis template identifier for item
                        templateName: '#tpl-etudiant-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.etudiant = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.etudiant,
                    // tpl-etudiant-details is template identifier for chosen model element
                    templateName: '#tpl-etudiant-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Etudiant();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-etudiant-details is a template identifier for chosen model element
                templateName: '#tpl-etudiant-details',
