...
// Model for Filiere entity
    models.Filiere = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/ServiceWebRESTInscription/api/filieres/",
        idAttribute: 'id',
        defaults: {
            titre: ""
        },
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/ServiceWebRESTInscription/api/filieres/';
            }
    ...
    // Collection class for Filiere entities
    models.FiliereCollection = Backbone.Collection.extend({
        model: models.Filiere,
        url: "http://localhost:8080/ServiceWebRESTInscription/api/filieres/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.FiliereCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-filiere-list-itemis template identifier for item
                        templateName: '#tpl-filiere-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.filiere = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.filiere,
                    // tpl-filiere-details is template identifier for chosen model element
                    templateName: '#tpl-filiere-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Filiere();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-filiere-details is a template identifier for chosen model element
                templateName: '#tpl-filiere-details',
